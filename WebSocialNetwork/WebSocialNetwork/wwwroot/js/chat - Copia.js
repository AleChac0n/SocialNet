$(document).ready(function () {
    ...
    function AppViewModel() {
        ...
        self.showRoomActions = ko.computed(function () {
            return self.joinedRoom()?.appUsuario1() == self.myProfile()?.userName();
        });
        ...
        self.createRoom = function (roomName, usuario) {
            fetch('/api/Rooms/', {
                ...
                .then(data => {
                    // Redirigir al usuario a la sala de chat específica
                    window.location.href = '/Chats?id=' + data.id;
                })
                .catch(error => {
                    console.error('Error:', error);
                    // Aquí puedes mostrar un mensaje de error al usuario si la creación de la sala falla
                });
        }
...
//    connection.on("newMessage", function (messageView) {
//        var isMine = messageView.fromUserName === viewModel.myProfile().userName();
//        var message = new ChatMessage(messageView.id, messageView.contenido, messageView.timestamp, messageView.fromUserName, messageView.fromFullName, isMine, messageView.avatar);
//        viewModel.chatMessages.push(message);
//        $(".messages-container").animate({ scrollTop: $(".messages-container")[0].scrollHeight }, 1000);
//    });
...
//                    method: 'POST',
//                    headers: { 'Content-Type': 'application/json' },
//                    body: JSON.stringify({ room: room.name(), contenido: message })
//                });
//            }
//        }
...
//                    self.chatRooms.removeAll();
//                    for (var i = 0; i < data.length; i++) {
//                        self.chatRooms.push(new ChatRoom(data[i].id, data[i].nombre, data[i].appUsuario1));
//                    }
...
//    function ChatMessage(id, contenido, timestamp, fromUserName, fromFullName, isMine, avatar) {
//        var self = this;
//        self.id = ko.observable(id);
